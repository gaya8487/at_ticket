...
public class ReservationController {
	...
	public BaseResponse<GetReservationSeatsResDto> getReservationSeats(@PathVariable("showId") Long showId) {
		...
		List<ReservedSeat> seats = reservedSeatRepository.findReservedSeatByShowId(showId);

		List<Long> seatIdList = seats.stream().map(x -> x.getSeatId()).collect(Collectors.toList());

		return ok(
			GetReservationSeatsResDto.builder()
				.showId(seats.get(0).getShowId())
				.seatIds(seatIdList)
				.build()
